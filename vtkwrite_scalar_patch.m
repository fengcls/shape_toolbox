function vtkwrite_scalar_patch(filename,mean_vertices,patch_Vertices,patch_Faces,scalars_name,scalars)
%% some old tries
...
%% here
[~,vert_ind] = pdist2(mean_vertices(1,:)',patch_Vertices(:,1),'euclidean','smallest',1);
vtk_scalar = scalars(vert_ind);
% use code migrated from others, attached below
vtkwrite(filename,'polydata','triangle',patch_Vertices(:,1),patch_Vertices(:,2),patch_Vertices(:,3),patch_Faces)
...
fprintf(fid,'%1$s\t%2$i\n','POINT_DATA',length(vtk_scalar));
fprintf(fid,'%s\n','FIELD FieldData 1');
fprintf(fid,'%1$s %2$i %3$s\n',[scalars_name,' 1'],length(vtk_scalar),'float');

for kfpr = 1:length(vtk_scalar)
    fprintf(fid,'%4.4f\t',vtk_scalar(kfpr));
    if (mod(kfpr,9)==0)
        fprintf(fid,'\n');
    end;
end;
