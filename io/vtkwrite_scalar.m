function vtkwrite_scalar(filename,vertex,face,scalars_name,scalar)
%% some old tries
...
if nargin==5
    ...
    fprintf(fid,'%1$s\t%2$i\n','POINT_DATA',length(scalar));
    fprintf(fid,'%s\n','FIELD FieldData 1');
    fprintf(fid,'%1$s %2$i %3$s\n',[scalars_name,' 1'],length(scalar),'float');
    
    for kfpr = 1:length(scalar)
        fprintf(fid,'%4.4f\t',scalar(kfpr));
        if (mod(kfpr,9)==0)
            fprintf(fid,'\n');
        end;
    end;
