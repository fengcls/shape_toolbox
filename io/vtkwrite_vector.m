function vtkwrite_vector(filename,vertex,face,vector_name,vector)
%% here
...
% guess have to write in the vector file information by myself
fid = fopen(filename,'a');

if nargin==5
    % at present only one vector is supported or is needed actually
    fprintf(fid,'\n');
    fprintf(fid,'%1$s\t%2$i\n','POINT_DATA',length(vector));
    fprintf(fid,'VECTORS\t%1$s\tfloat\n',vector_name);
    
    precision = '3';
    vector_to_write = [vector(1:3:end-2,1) vector(1:3:end-2,2) vector(1:3:end-2,3) ...
        vector(2:3:end-1,1) vector(2:3:end-1,2) vector(2:3:end-1,3) ...
        vector(3:3:end,1) vector(3:3:end,2) vector(3:3:end,3)]';
    spec = [repmat(['%0.',precision,'f '],1,9),'\n'];
    fprintf(fid,spec, vector_to_write);   
end;
fclose(fid);
end
