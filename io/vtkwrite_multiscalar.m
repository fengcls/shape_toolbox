function vtkwrite_multiscalar(filename,vertex,face,scalars_name,scalars)
%% some old tries
% actually good, but no vtk format
% write_mesh('testtestvtk_mean2.obj',mean_vertices,faces_list)
% good but cannot add scalar?
% vtkwrite('testtestvtk_mean.vtk','polydata','triangle',mean_vertices(1,:),mean_vertices(2,:),mean_vertices(3,:),faces_list')
% similar to the one above
% vtkwrite('testtestvtk_mean.vtk','polydata','triangle',p1.Vertices(:,1),p1.Vertices(:,2),p1.Vertices(:,3),p1.Faces)

if ~iscell(scalars)
    disp('use vtkwrite_scalar instead');
    vtkwrite_scalar(filename,vertex,face,scalars_name,scalars)
    return;
end;

num_scalar_field = length(scalars);
num_vertices = length(scalars{1});
%% here
% use code migrated from others, attached below
if (size(face,1)==3)
    face = face';
end;
...
vtkwrite(filename,'polydata','triangle',vertex(:,1),vertex(:,2),vertex(:,3),face)
% guess have to write in the scalar file information by myself
fid = fopen(filename,'a');

if nargin==5
    % at present only one scalar is supported or is needed actually
    fprintf(fid,'%1$s\t%2$i\n','POINT_DATA',num_vertices);
    fprintf(fid,'%1$s %2$i\n','FIELD FieldData',num_scalar_field);
    
    for kscalar = 1:length(scalars)
        fprintf(fid,'%1$s %2$i %3$s\n',[scalars_name{kscalar},' 1'],num_vertices,'float');
        
        for kfpr = 1:length(scalars{kscalar})
            fprintf(fid,'%4.4f\t',scalars{kscalar}(kfpr));
            if (mod(kfpr,9)==0)
                fprintf(fid,'\n');
            end;
        end;
